...
MAIL_USERNAME = "client@local.dev"
...
async def get_emails(folder="INBOX"):
    ...
    # GreenMail might not support SSL on this port, so we might need to use starttls or just plain IMAP.
    # Standard aioimaplib uses SSL by default.
    # Let's try non-SSL first since the config suggests plain ports (3143).
    # aioimaplib.IMAP4 is for non-SSL.
    ...
    try:
        ...
        # data[0] is a space-separated list of message numbers in bytes
        ...
        for num in msg_nums:
            ...
            # data[1] is the content usually
            # aioimaplib returns a list where the email body is in the second element of the tuple usually
            # Structure is complex, let's parse carefully.
